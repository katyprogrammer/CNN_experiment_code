...
def build_mlp(input_var=None):
    ...
    conv3 = lasagne.layers.Conv2DLayer(
        maxpool2, name='conv_3', num_filters=32,
        filter_size=(5, 5),
        nonlinearity=lasagne.nonlinearities.rectify
    )
    maxpool3 = lasagne.layers.MaxPool2DLayer(
        conv3, name='maxpool_3', pool_size=(2, 2)
    )
    conv4 = lasagne.layers.Conv2DLayer(
        maxpool3, name='conv_4', num_filters=32,
        filter_size=(5, 5),
        nonlinearity=lasagne.nonlinearities.rectify
    )
    maxpool4 = lasagne.layers.MaxPool2DLayer(
        conv4, name='maxpool_4', pool_size=(2, 2)
    )
    conv5 = lasagne.layers.Conv2DLayer(
        maxpool4, name='conv_5', num_filters=32,
        filter_size=(5, 5),
        nonlinearity=lasagne.nonlinearities.rectify
    )
    maxpool5 = lasagne.layers.MaxPool2DLayer(
        conv5, name='maxpool_5', pool_size=(2, 2)
    )
    dropout1 = lasagne.layers.DropoutLayer(
        maxpool5, name='dropout_1', p=0.5
    )
    ...
    dense2 = lasagne.layers.DenseLayer(
            dense1, name='dense_2', num_units=150,
            nonlinearity=None
    )
    dense3 = lasagne.layers.DenseLayer(
            dense2, name='dense_3', num_units=150,
            nonlinearity=None
    )
